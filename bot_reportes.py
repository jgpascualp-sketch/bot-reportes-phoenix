...
# Conexión opcional a Google Sheets para autocompletar si existen las credenciales
...
def buscar_datos_hospital(nombre_hosp):
    try:
        ...
        logger.error(f"Error al consultar Sheets para autocompletar: {e}")
    return None
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "👋 ¡Hola Jesús! Usa /reporte para generar un nuevo REPORTE DE SERVICIO TÉCNICO en documento."
    )
...
async def get_hospital(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sug:
        ...
        reply_markup = ReplyKeyboardMarkup(teclado, one_time_keyboard=True, resize_keyboard=True)
        await update.message.reply_text(
            f"💡 Datos encontrados para este hospital:\n"
...
async def get_serie(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    teclado = [["Dejar vacío"]]
    reply_markup = ReplyKeyboardMarkup(teclado, one_time_keyboard=True, resize_keyboard=True)
    await update.message.reply_text("⏱️ Paso 8/14: Ingrese Horómetro (Running hours) o pulse el botón:", reply_markup=reply_markup)
    return HOROMETRO
...
async def get_moneda(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("⏳ Generando tu reporte técnico con firma...")
    
    # Ensamblar documento Word
    ruta_plantilla = "1-TECHNICAL SERVICE REPORT.docx"
    ...
    # Rellenar firmas y nombres sincronizados
    t2 = doc.tables[1]
    ...
    # Insertar firma transparente fija
    ...
    with open(nombre_archivo, "rb") as f:
        await context.bot.send_document(
            ...
            document=f,
            caption="✅ ¡Tu Reporte de Servicio Técnico ha sido generado con éxito!"
        )
...
def main():
    ...
    logger.info("Bot de Reportes iniciado...")
    app.run_polling()
